...
const weapons = [
  ...
  🇷🇺 Определяем массив объектов weapons, где каждый из объектов описывает оружие и его свойства (название и сила). Каждый объект будет иметь 2 свойства: name и power. То есть, каждый элемент массива weapons представляет собой объект, который содержит два свойства: name (название оружия) и power (сила атаки). */
  {
...
const monsters = [
  /* 🇦🇺 Define the array of objects that represent different monsters that the player can encounter, each with their own name, level, and health. 
🇷🇺 Это массив объектов (array of objects), которые представляют различных монстров, каждый со своим именем, уровнем и здоровьем. */
  ...
  {
    name: "fanged beast", // 🇷🇺 Клыкастый зверь
    level: 8,
    health: 60,
  },
  {
    name: "Zomby", // 🇷🇺 Классические зомби - мертвецы, которые оживают и ходят, не обладая разумом и контролем над своими действиями.
    level: 9,
    health: 70,
  },
  {
    name: "Woodoo", // 🇷🇺 Шаманский зомби - зомби, созданный с помощью магии или ритуалов.
    level: 10,
    health: 200,
  },
  {
    name: "Walking Dead", // 🇷🇺 Нежить - персонажи, которые вернулись из мертвых, но они могут обладать силой и интеллектом, превосходящими человеческие.
    level: 11,
    health: 210,
  },
  {
    name: "Grass", // 🇷🇺 Растительные зомби - зомби, созданные в результате заражения растениями или грибами. Они могут быть медленными и неуклюжими, но также могут обладать сверхъестественными способностями.
    level: 12,
    health: 220,
  },
  {
    name: "Infected", // 🇷🇺 Инфицированные зомби - люди, зараженные вирусом или другим заболеванием, которое превращает их в агрессивных и безумных существ.
    level: 13,
    health: 230,
  },
  {
    name: "Underground Zombie", // 🇷🇺 Подземный зомби - это зомби, который обитает под землей, например, в катакомбах или туннелях.
    level: 14,
    health: 240,
  },
  {
    name: "Bloodsucker", // 🇷🇺 Кровососущий - это зомби, который питается кровью живых существ, в том числе и людей.
    level: 15,
    health: 250,
  },
  {
    name: "Mutant", // 🇷🇺 Мутант - это зомби, который мутировал в результате воздействия радиации или других опасных веществ.
    level: 16,
    health: 260,
  },
  {
    name: "Winged", // 🇷🇺 Крылатый - это зомби, который может летать и атаковать своих жертв с высоты.
    level: 17,
    health: 270,
  },
  {
    name: "Infected", // 🇷🇺 Инфицированный (Infected) - это зомби, созданный в результате инфекции, например, вируса.
    level: 18,
    health: 280,
  },
  {
    name: "Ghoul", // 🇷🇺 Гул - это зомби-людоед, который обитает в гробницах и подземельях и питается трупами.
    level: 19,
    health: 290,
  },
  {
    name: "Runner", // 🇷🇺 Это быстрый и опасный зомби, который быстро бегает к своей жертве и атакует ее когтями.
    level: 20,
    health: 300,
  },
];
...
/*
1) Рунер (Runner) - это быстрый и опасный зомби, который быстро бегает к своей жертве и атакует ее когтями.

2) Гул (Ghoul) - это зомби-людоед, который обитает в гробницах и подземельях и питается трупами.

3) Инфицированный (Infected) - это зомби, созданный в результате инфекции, например, вируса.

4) Крылатый (Winged) - это зомби, который может летать и атаковать своих жертв с высоты.

5) Мутант (Mutant) - это зомби, который мутировал в результате воздействия радиации или других опасных веществ.

6) Кровососущий (Bloodsucker) - это зомби, который питается кровью живых существ, в том числе и людей.

7) Подземный зомби (Underground Zombie) - это зомби, который обитает под землей, например, в катакомбах или туннелях. 

8) Классические зомби - мертвецы, которые оживают и ходят, не обладая разумом и контролем над своими действиями.

9) Инфицированные зомби - люди, зараженные вирусом или другим заболеванием, которое превращает их в агрессивных и безумных существ.

10) Растительные зомби - зомби, созданные в результате заражения растениями или грибами. Они могут быть медленными и неуклюжими, но также могут обладать сверхъестественными способностями.

11) Нежить - персонажи, которые вернулись из мертвых, но они могут обладать силой и интеллектом, превосходящими человеческие.

12) Шаманский зомби - зомби, созданный с помощью магии или ритуалов.

13) Ходячий скелет (Skeleton) - это зомби, состоящий из костей и без мяса и кожи.

14) Жидкостный зомби (Liquid Zombie) - это зомби, который состоит из жидкости, обычно токсичной или радиоактивной.

15) Рыбный зомби (Zombie Fish) - это зомби, который был рыбой до своего превращения и обитает в воде.

16) Гигантский зомби (Giant Zombie) - это зомби, который гигантских размеров и обычно представляет угрозу для целых групп людей.

17) Раковый зомби (Zombie Crab) - это зомби, который был раком до своего превращения и обитает на побережье.

18) Гниющий зомби (Decaying Zombie) - это зомби, который уже давно мертв и находится в состоянии сильного распада.

19) Роботизированные зомби - создания, которые были роботизированы и контролируются внешними силами. 

20) Падший ангел (Fallen Angel) - это зомби, который был некогда ангелом, но потерял свою небесную сущность и стал смертельным для людей. 

21) Химический зомби - это зомби, созданный с помощью химических веществ, например, газа.

22) Гипнотический зомби - это зомби, который подчинен воле своего создателя и может быть использован для выполнения различных задач.

23) Астральный зомби (Astral Zombie) - это зомби, который был оживлен духами или магической силой, и может быть управляем только опытным магом.

24) Радиоактивный зомби - это зомби, созданный в результате воздействия радиации на организм.

25) Зомби-убийца (Zombie Slayer) - это человек, который специализируется на уничтожении зомби и защите людей от них.

26) Механический зомби - это зомби, который был создан с помощью механики или робототехники, и может быть управляем только компьютером.

27) Зомби-вампир (Zombie Vampire) - это зомби, который питается кровью и мясом своих жертв, подобно вампирам.

28) Призрачный зомби (Ghost Zombie) - это зомби, который был оживлен призраками или духами, и обладает сверхъестественными способностями.

29) Зомби-собиратель (Zombie Collector) - это человек, который специализируется на сборе зомби для использования их в различных целях, например, в качестве рабочей силы или для проведения опытов.

30) Кибернетический зомби - это зомби, который был улучшен с помощью кибернетики или искусственного интеллекта, и может быть управляем только компьютером или оператором. 
*/
